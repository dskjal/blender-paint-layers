...
class MergeVisibleLayers(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bake_type_str = 'EMIT' if  bpy.context.object.active_material.layer_use_emission else 'DIFFUSE'

        # Bake layers
        img_data = context.object.active_material.layer_img_data
        img = bpy.data.images.new(name="merged_layers", width=img_data.width, height=img_data.height, float_buffer=img_data.bit_depth)
        ...
        scene.cycles.bake_type = bake_type_str
        scene.render.bake.use_pass_direct = False
        scene.render.bake.use_pass_indirect = False
        bpy.ops.object.bake(type=bake_type_str)
