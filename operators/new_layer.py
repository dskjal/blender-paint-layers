...
def change_shader():
    ...
    # Base Color to Black
    mat_output.inputs[0].default_value = (0, 0, 0, 1)

    # find mix node
    mix = None
    for l in tree.links:
        if l.to_node.name == mat_output.name and 'LAYER_MIX' in l.from_node.name:
            mix = l.from_node
            tree.links.remove(l)
            break

    if mix is not None:
        use_emission = bpy.context.object.active_material.layer_use_emission
        tree.links.new(mix.outputs[0], mat_output.inputs[17 if use_emission else 0])
...
def setup_output(material, tree, offset):
    ...
    diffuse = tree.nodes.new('ShaderNodeBsdfPrincipled')
    diffuse.name = "LAYER_DIFFUSE"
    material.layer_shaders.diffuse = diffuse.name
